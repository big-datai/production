...
def lint_clip(clip: dict):
    ...
    # R30. Every clip with native audio MUST have explicit quoted dialogue.
    # New hard rule 2026-05-11: silence directives produce "strange sounds"
    # (random mumbling, garbled ambient) — observed empirically. The ONLY way
    # to get clean audio out of kling-v3-omni is to give it specific lines to say.
    # For "quiet" beats (night-falls, transitions), add a whispered narrator line
    # or character thought; for true silence, set sound:"off" explicitly.
    # Memory: lesson_kling_require_clear_speech.md.
    if clip.get("nativeAudio"):
        has_dialogue = bool(re.search(r'"[^"]{2,}"', prompt))
        if not has_dialogue:
            findings.append(("error",
                "R30 nativeAudio=true but NO quoted dialogue in prompt. Hard rule "
                "(2026-05-11): every clip needs spoken lines or Kling generates "
                "strange/garbled sounds. Add a Char(tone):\"...\" line (whisper or "
                "narration is fine), or set nativeAudio=false explicitly."))
