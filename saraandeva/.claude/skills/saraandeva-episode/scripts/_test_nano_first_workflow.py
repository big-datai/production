...
def split_into_states(canonical_prompt: str) -> tuple[str, str, str]:
    ...
    Heuristic:
      - First 1-2 sentences = scene establishing + opening positions = START STATE
      - Middle sentences with verbs/body-parts = ACTION SEQUENCE
      - Final 1-2 sentences with positional terminus = END STATE

    If the prompt is short (single sentence), repeat as both start and end.
    """
    # Strip @char anchors — Nano renders by character name directly
    text = re.sub(r"@(\w+)", r"\1", canonical_prompt).strip()
    # Strip Cast LOCKS section if present
    ...
    if len(sentences) <= 2:
        return sentences[0], " ".join(sentences), sentences[-1]

    # First sentence(s) = setup / opening positions
    # Last sentence(s) = closing positions / payoff frame
    start_sentences = sentences[:1]
    end_sentences = sentences[-1:]
    # Extend if first sentence is very short
    if len(start_sentences[0]) < 80 and len(sentences) > 3:
        start_sentences.append(sentences[1])
    if len(end_sentences[0]) < 80 and len(sentences) > 3:
        end_sentences.insert(0, sentences[-2])

    start = " ".join(start_sentences)
    end = " ".join(end_sentences)
    action = " ".join(sentences[len(start_sentences):len(sentences) - len(end_sentences)])
